...
class MatchActivity : AppCompatActivity() {
    ...
    private fun battle() {
        ...
        if (weaponPlayer1 != null && weaponPlayer2 != null) {
            winner = match.toPlay(weaponPlayer1!!, weaponPlayer2!!)
            if (winner != null) {
                Toast.makeText(
                    this,
    ...
    private fun startSelectionActivity(number: Int) {
        val name = if (number == 1) match.player1.name else match.player2.name
    ...
    private fun updateUI() {
        ...
        binding.labelPlayer1.text = match.player1.name
        binding.labelPlayer2.text = match.player2.name
        updateScoreBoard()

        binding.buttonWeapon1.text = "${match.player1.name} ${getString(R.string.gun_selection)}"
        binding.buttonWeapon2.text = "${match.player2.name} ${getString(R.string.gun_selection)}"
    }
}